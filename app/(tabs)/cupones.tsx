...
import { collection, query, onSnapshot, doc, setDoc, addDoc, deleteDoc, updateDoc, serverTimestamp, Timestamp, getDocs, where } from 'firebase/firestore';
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId) {
        ...
        const discountText = type === 'percentage' ? `${parsedValue}% de descuento` : `$${parsedValue.toLocaleString("de-DE")} de descuento`;
        await addDoc(collection(db, 'notificaciones'), {
          tipo: 'global',
          titulo: '🏟️ ¡Nuevo cupón disponible!',
          mensaje: `Usa el código ${code.toUpperCase().trim()} y obtén ${discountText} en tu próxima compra${parsedMinAmount > 0 ? ` (mín. $${parsedMinAmount.toLocaleString("de-DE")})` : ''}.`,
          codigo: code.toUpperCase().trim(),
          leidaPor: [],
          creadaEn: Timestamp.now(),
        });

        // Mass Push Notification to non-admin users
        try {
          ...
          if (!usersSnap.empty) {
            ...
            await addDoc(collection(db, 'ff_user_push_notifications'), {
              ...
              notification_text: `¡Nuevo cupón! Usa ${code.toUpperCase().trim()} para obtener ${discountText}.`,
              notification_title: '🎟️ ¡Nuevo Cupón de Descuento!',
              num_sent: usersSnap.size,
              parameter_data: JSON.stringify({ couponCode: code.toUpperCase().trim() }),
              sender: doc(db, 'users', 'admin'),
              status: 'pending',
              app_target: 'tienda',
          ...
          console.error('Error sending coupon push:', pushErr);
        }
            ...
            <View key={c.id} style={[styles.couponCard, { width: isDesktop ? '31%' : '100%' }]}>
              <View style={styles.cardHeader}>
                <View style={[styles.statusBadge, { backgroundColor: c.active ? '#ECFDF5' : '#F1F5F9' }]}>
                  <View style={[styles.statusDot, { backgroundColor: c.active ? '#63348C' : '#94A3B8' }]} />
              ...
              <View style={styles.mainInfo}>
                <Text style={styles.discountDisplay}>
                  {c.type === 'percentage' ? `${c.value}%` : `$${c.value.toLocaleString("de-DE")}`}
              ...
              <View style={styles.cardFooter}>
                <View style={styles.footerItem}>
                  ...
                  <Text style={styles.footerLabel}>Mín: <Text style={styles.footerValue}>${c.minAmount?.toLocaleString("de-DE") || '0'}</Text></Text>
                </View>
                {c.expiryDate && (
                  <View style={styles.footerItem}>
...
const styles = StyleSheet.create({
  ...
  addBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#10B981', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, gap: 6, shadowColor: '#10B981', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.2, shadowRadius: 12, elevation: 4 },
  addBtnText: { color: '#fff', fontWeight: '800', fontSize: 13 },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 24 },
  
  couponCard: { backgroundColor: '#fff', borderRadius: 24, padding: 20, borderWidth: 1, borderColor: '#F1F5F9', shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.04, shadowRadius: 30 },
  ...
  discountDisplay: { fontSize: 28, fontWeight: '900', color: '#0F172A', letterSpacing: -0.5 },
  discountSub: { fontSize: 14, color: '#64748B', fontWeight: '700' },
  codeContainer: { marginTop: 10, backgroundColor: '#F8FAFC', paddingHorizontal: 14, paddingVertical: 6, borderRadius: 10, flexDirection: 'row', alignItems: 'center', gap: 6, borderWidth: 1, borderColor: '#E2E8F0', borderStyle: 'dashed' },
  couponCodeText: { fontSize: 14, fontWeight: '800', color: '#63348C', letterSpacing: 0.5 },
  copyBtn: { padding: 4 },
  ...
  emptyBtn: { marginTop: 24, backgroundColor: '#10B981', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 12 },
  ...
  input: { backgroundColor: '#F8FAFC', borderRadius: 12, padding: 12, borderWidth: 1, borderColor: '#E2E8F0', fontSize: 14, color: '#0F172A', fontWeight: '700' } as any,
  ...
  typeBtnTextActive: { color: '#63348C' },
